...
import { COLUMNS } from './TableOrdersAdminCompleted__Columns';

interface iTableOrders {
  orders: any[];
}

export default function TableOrdersAdminCompleted({ orders }: iTableOrders) {
  ...
  return (
    <>
      {localStorage.getItem('terminalUserID') !== '1343' && (
        <Button
          type='button'
          onClick={() =>
            router.push(makeContextualHref({ modal: 'xlsx' }), `${router.pathname}`, {
              shallow: true,
            })
          }
          loaderColor='red'
