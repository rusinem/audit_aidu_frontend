...
import TableOrdersAdminInWork from '../../../../components/TableOrdersAdminInWork';
...
const OrdersInWork = () => {
  ...
  const {
    ...
    ['ordersInWork', router.query.lang],
    ({ pageParam = 1 }) => api_ordersAdmin({ status: '3, 7', sort: 'desc', page: pageParam, country: router.query.lang as string }),
    {
      ...
      <main>
        ...
        {ordersStatus === 'loading' || userStatus === 'loading' ? (
          ...
          <div className='container pl-50 pb-30 relative -mb-180'>
            ...
            <TableOrdersAdminInWork orders={mergeOrdersPages(orders) || []} />

            <div className='text-16 leading-20 font-medium absolute bottom-[240rem] right-[50rem]'>
              Всего заказов в работе {hasNextPage ? `~ ${orders.pages[0].total_pages * 50}` : `- ${mergeOrdersPages(orders).length}`}
            </div>
